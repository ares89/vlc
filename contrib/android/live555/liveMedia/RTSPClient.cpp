...
int RTSPClient::openConnection()
{
	...
	serverPort=(unsigned short)urlPortNum;
        ...
        if((index=strstr(strdup(urlSuffix),"rtspport"))!=NULL)
        {
            ...
            if(*index=='=')
            {
                sscanf(++index,"%d",&rtspPort);
                if (rtspPort < 1 || rtspPort > 65535)
                {
                    envir() << "the udp port "<<rtspPort<<" is not valid"<<"\n";
                    break;
                }
                else
                {
        ...
        index=0;
        if((index=strstr(strdup(urlSuffix),"rtpwanport"))!=NULL)
        {
            index+=10;
            if(*index=='=')
            {
        ...
        index=0;
        if((index=strstr(strdup(urlSuffix),"audiowanport"))!=NULL)
        {
            index+=12;
            if(*index=='=')
            {
        ...
        if (fVerbosityLevel >= 1)
        {
            ...
            if(isUDP)
            {
                envir() << "this is a rtsp-over-udp server. my rtsp port will be "<<rtspPort<<".\n";
            }
        }
        ...
        //change upper line by lishug select the type of socket here
        if(isUDP==false)
            ...
            fInputSocketNum = fOutputSocketNum = setupDatagramSocket(envir(), rtspPort);
...
unsigned RTSPClient::sendRequest(RequestRecord* request)
{
    ...
    do
    {
		...
		char* tempindex;
//	file<<"cmdURL:"<<cmdURL<<" rtpwanport:"<<rtpWanPort<<"\n";

     //delete the suffix &rtpport=xxxx which will be add at the last of url
 		if((tempindex=strstr(cmdURL,"rtpport"))!=NULL)
        {
            *(--tempindex)='\0';
        }
