...
int RTSPClient::openConnection()
{
    do
    {
        ...
        char* index=0;
        if((index=strstr(strdup(urlSuffix),"udpport"))!=NULL)
        {
            index+=7;
            if(*index=='=')
            {
                sscanf(++index,"%d",&myUDPPort);
                if (myUDPPort < 1 || myUDPPort > 65535)
                {
                    envir() << "the udp port "<<myUDPPort<<" is not valid"<<"\n";
                    break;
                }
                else
                {
        ...
        if (fVerbosityLevel >= 1)
        {
            ...
            if(isUDP)
            {
                envir() << "this is a rtsp-over-udp server. my udp port will be "<<myUDPPort<<".\n";
            }
        }
        ...
        //change upper line by 李曙光 select the type of socket here
        if(isUDP==false)
            ...
            fInputSocketNum = fOutputSocketNum = setupDatagramSocket(envir(), myUDPPort);
...
unsigned RTSPClient::sendRequest(RequestRecord* request)
{
    ...
    do
    {
        ...
        //delete the suffix &udpport=xxxx which will be add at the last of url
        char* tempindex;
        if((tempindex=strstr(cmdURL,"udpport"))!=NULL)
        {
            *(--tempindex)='\0';
        }
        if (fVerbosityLevel >= 1) envir() << cmdURL<<".\n";
        //delete the suffix &udpport=xxxx which will be add at the last of url
...
void RTSPClient::handleResponseBytes(int newBytesRead)
{
    ...
    do
    {
        ...
        do
        {
            ...
            {
                envir()<<"the foundRequest is null"<<"\n";
                ...
                envir()<<(foundRequest != NULL ? foundRequest->commandName() : "(unknown)")<<"\n";
            }
            ...
            envir() << "Received a complete1 ";
            if (foundRequest != NULL)
            {
                envir() << "Received a complete2";
